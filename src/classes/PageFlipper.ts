import { PageFlipperOptions, PageElement } from '../types/index.js';
...
export class PageFlipper {
  ...
  private backgroundSlides: NodeListOf<HTMLElement>;
  private currentSlide: number = 0;
  private wheelTimeout?: number;

  constructor(options: PageFlipperOptions = {}) {
  ...
  private bindEvents(): void {
    ...
    // Buttons
    document.querySelectorAll<PageElement>('.btn').forEach(btn => {
      if (btn.dataset.page) {
   ...
   * Initialize background slideshow
   */
  private initBackgroundSlideshow(): void {
    if (this.backgroundSlides.length === 0) return;
   ...
   */
  private nextBackgroundSlide(): void {
    if (this.backgroundSlides.length === 0) return;
